...
class PybulletRGBDSensor(PybulletSensor):
    ...
    def convert_camera_image_to_pointcloud(self, width, height, rgbPixels, depthPixels, segmentationMaskBuffer):
        msg = PointCloud2()
        msg.header.stamp = self.node.time_now()
        msg.header.frame_id = 'rpbi/world'
        # >>>>>>>>>>>>>TODO<<<<<<<<<<<<
        return msg
    ...
    def main_loop(self, event):
        ...
        self.pubs['pointcloud'].publish(
            self.convert_camera_image_to_pointcloud(width, height, rgbPixels, depthPixels, segmentationMaskBuffer)
        )
