...
from py_pack import hybridto_dac_so
...
class PlanInterpWithTO:
    ...
    def buildTO(self):
        ...
        self.HybOpt3D = hybridto_so.HybridTOClass3D()

        self.HybOpt_DAC = hybridto_dac_so.HybridTOClass_DAC()
...
class PredictionWithTO():
    def __init__(self, initBaseYang, initBaseYin, paramFilePath = 'configs/config_prediction.yaml',
                 initFilePath = 'data/init_prediction.npy'):
        # Name of node
        self.name = rospy.get_name()
        ...
        # initial swinging conditions
        self.hangPoint = np.array(params['hangPoint'])
        self.ropeLength = params['ropeLength']
        self.startAngle = params['startAngle']
    ...
    def buildTO(self):
        ...
        #  create instance of the Hybrid optimisation class
        self.HybOpt3D_SRB = hybridto_so.HybridTOClass3D()
    ...
    def solveTO(self, initObjPos, initObjVel):
        ...
        if (preFlag):
            ...
            initIndex, normIndex, normVector = self.HybOpt3D_SRB.contactSelection(posBodyPre, self.r, self.L0, self.R0)
...
if __name__=='__main__':
    ...
    if objectState == "Swing":
        ...
        if ori_representation == "euler":
            # euler representation initialization #
            initObjPos = np.array([0, (PredictionWithTO.ropeLength + PredictionWithTO.objLength / 2) * np.sin(
                PredictionWithTO.startAngle * np.pi / 180.), PredictionWithTO.hangPoint[2]
                                   - (PredictionWithTO.ropeLength + PredictionWithTO.objLength / 2) * np.cos(
                PredictionWithTO.startAngle * np.pi / 180.), 0, PredictionWithTO.startAngle * np.pi / 180., 0])

            pos = initObjPos[0:3]
            quat = R.from_euler('ZYX', initObjPos[3:6]).as_quat()
        elif ori_representation == "quaternion":
            # quaternion representation initialization #
            initObjPos = np.array([0, 0, 0.0, 0, 0, 0, 1])
            pos = initObjPos[0:3]
            quat = initObjPos[3:7]
