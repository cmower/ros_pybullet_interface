...
if __name__ == '__main__':
    ...
    #  goal position of the end_effector in the world frame
    pos_initial = np.array([0.15, 0.45, 0.6221])

    #  goal orientation of the end_effector in the world frame
    #  in quaternions
    orient_initial = np.array([0.866, -0.497, 0.022, -0.0377])

    #  duration of motion... the shorter the duration, the faster the trajectory
    #  to the target
    #  the 1st is the beginning of the trajectory and
    #  the 2nd automatically computed such that the beginning of the motion is very slow
    motion_dur = 10.0
    timeSeq = np.array([0.0, 0.2*motion_dur, motion_dur])
